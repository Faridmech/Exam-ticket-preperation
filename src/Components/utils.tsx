...
export const generatePdf =  ({data, headers, filename}: IArgs)  => {
  const doc = new jsPDF({
    orientation: 'l',
    unit: 'pt',
    format: 'a4',
  ...
  data.forEach((row, rIndex) => {
    ...
    * */
    ...
    // COLUMNS
    headers.forEach((column, cIndex: number) => {
      const longText = doc.splitTextToSize(String(row[column.key]), xPositions[cIndex] - xPositions[(cIndex !== 0 && cIndex - 1) as number] )
